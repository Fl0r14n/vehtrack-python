...
angular.module('utils').directive('uiTabList', function () {
    return {
        ...
        template: [
            ...
            '               <li ng-repeat="tab in tabsCtrl.getTabs()" ng-click="tabsCtrl.setActiveTab(tab)">',
            '                   <a ng-class="{\'active\': tabsCtrl.isActiveTab(tab), \'disabled\': !tabsCtrl.isActiveTab(tab)}"><i ng-if="tab.icon" class="{{ tab.icon }}"></i>{{ tab.name }}<button ng-if="tabsCtrl.isClosable()" class="close" ng-click="tabsCtrl.closeTab(tab)">&times;</button></a>',
            '               </li>',
            '           </ul>',
            '       </div>',
